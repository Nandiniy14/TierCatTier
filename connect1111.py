...
def addclient():
	...
	addCat.insert({ "clientName":values['name'],"clientFeesOwner":None,"email":"","secondaryEmail":"","createdBy":"",\
		...
		"carrierID":"","exchangeRate":"","tiers":[],"cat_fees":{},"static_fees":{'admin':"",
				'itv':"",
				'photos':"",
				'state_ny':"",
				'reviewer':"",
...
@app.route('/getCatTierFees',methods=['GET'])
def getCatTierFees():
	client_name = request.args.get('data')
	print(client_name)
	users = db.details.find_one({"clientName" : client_name})
	data = []
	data.append({'cat_fees':users['cat_fees']})
	print(data)
	return jsonify(data)
...
def addCatTierFees():
	...
	cat_fees[values['catTier']] = [{'min': 1000}]
...
def addCatTierData():
	...
	addCatTier.update({"_id":itm.get('_id')}, {'$push': {
		'cat_fees.{}'.format(cat_tier_name):  values
	}})
	return "success"
