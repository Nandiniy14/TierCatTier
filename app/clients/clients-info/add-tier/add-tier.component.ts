...
export class AddTierComponent implements OnInit,OnChanges {
  ...
  // @Input('selectedCatTier') catTierFee: string;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    // this.catTierFee = changes['selectedCatTier'].currentValue;
    // console.log(this.catTierFee+"CattierFee Name");
...
addTier() {
  ...
  onSave(){
    for(var i=0; i< this.myForm.value['add'].length; i++){
      ...
      // }
      alert("Please Enter tier values");
      }
      else {
